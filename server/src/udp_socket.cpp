...
udp_socket::udp_socket(const std::string & ip, const int port) : _ip(ip), _port(port)
{
    ...
    if (bind(_sckt, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
        close(_sckt);
        throw std::string("Error ! Cannot bind the socket.");
    }
    if (getsockname(_sckt, (struct sockaddr*)&addr, &addressLength) < 0) {
        close(_sckt);
        throw std::string("Error ! Cannot getsockname.");
    }
...
bool udp_socket::writeSocket(udp_socket::Datagram& datagram)
{
